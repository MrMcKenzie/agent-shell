...
For Cline coding agent, install with:

#+begin_src bash
npm install -g cline
#+end_src

See https://docs.cline.bot/cline-cli/ for details.
...
*** Auggie For login-based authentication (default):
...
For Cline CLI, install with:

#+begin_src bash
npm install -g cline
#+end_src

Cline is expected to be set up outside of Emacs using `cline auth` and can be optionally further configured using `cline config` and/or environment variables. Alternatively, you can use the VSCode Cline extension for setup.
For more detailed configuration information, visit: https://docs.cline.bot/cline-cli/getting-started

*** Customizing Available Agents

By default, =agent-shell= includes configurations for all supported agents (Claude Code, Gemini CLI, Codex, Goose, Qwen Code, Auggie, and Cline). You can customize which agents are available through the =agent-shell-agent-configs= variable.
